...
class AccountDepositModelForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "amount": forms.NumberInput(attrs={"class": "form-control py-3", "placeholder": "Enter amount"}),
            "reason": forms.Textarea(attrs={"class": "form-control", "rows": 4, "placeholder": "Reason for deposit"})
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if profile.plans.exists():
            ...
            self.fields["account"].initial = Account.objects.filter(pool__profile=profile).first()
