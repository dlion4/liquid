...
from amiribd.invest.models import Plan, Account, PlanType
...
class DashboardViewMixin(TemplateView):
    ...
    def __current_month_transactions(self):
        ...
        ### TODO: Renaming this function
         - Rename this here and in `__current_date_transaction_profit` and `__current_month_transactions`
        """
    ...
    def get_account_total(self, **kwargs)->Decimal:
        """
            ...
            ### Parameters:
                - kwargs (dict): Additional keyword arguments that may be used in future extensions.
            ###   Returns:
                - Decimal: The total balance of the user's account. If the account does not exist or an error occurs, it returns 0.00.
        """
        try:
    ...
    def get_monthly_site_investments(self):
        """
        This will return the current month's transaction and compare it with the previous month's deposit transaction 
        ...
        previous_month_deposits = self._get_monthly_deposits(previous_month_start, previous_month_end)
    ...
    def _get_monthly_deposits(self, start_date, end_date=None):
        ...
        ).prefetch_related("profile").prefetch_related("account").aggregate(total_deposits=Sum('paid'))['total_deposits'] or 0
    ...
    def get_total_site_investment(self)->Decimal:
        """
        This function will help in the retrieval of the toal ammount deposited into this site
        
        It takes the Transaction and then summ together all the amount that hassed been paid

        :TODO: In most cases will need a model to keep tranct of all the amount paid and those withdrawn

        For now just use the account |Transaction
            args: Transaction.objects.filter(type="Deposit").aggregate(investments=Sum("paid"))['investment']
                -> Optional: can filter for the account registration or not | IN this case it has been egnored
                -> Decimal(0.00) + Decimal(self.get_total_whatsapp_earnings())
                -> Decimal(0.00) + Decimal(self.get_total_site_investment())
        """
        investments =  Transaction.objects.filter(type="DEPOSIT").prefetch_related("profile").aggregate(investments=Sum("paid"))['investments']
    ...
    def get_total_paid_profit(self)->Decimal:
        ...
        For now just use the account |Transaction
        """
        profits =  Transaction.objects.filter(source__icontains="Referral Earnings").prefetch_related("profile").prefetch_related("account").aggregate(profits=Sum("paid"))['profits']
    ...
    def site_plan_population(self):
        ...
        This returns the plan population and polarity
         ...
         -  look for plan that have the plan type and count them and they should not be duplicate 
         -  example i have 3 plantype basic, standrd and gold,  many users can have basic plan but the type is one
         -  so i need the plan total per plantype
         -  the also find whihc user have plasn
         -  that have basic plan and have only one basic plan
         -  the same for standrd and gold
         -  profiles_with_plans = Profile.objects.filter(plans__isnull=False)
         -  total_profile_in_the_website = Profile.objects.all()
        
        """

        plan_counts = Plan.objects.values('type__type').annotate(total_plans=Count('id', distinct=True))
        ...
        for plan in plan_counts:
            plan['percentage_of_total'] = f"{((plan['total_plans'] / total_sum_plans) * 100):.0f}" if total_sum_plans != 0 else 0
    ...
    def get_context_data(self, **kwargs)->dict[str,Any]:
        ...
        Updates the context with data from the helper methods: get_profile_context, get_account_context, get_plans_context, get_transactions_context, and get_earnings_context.
        ...
        def get_context_data(self, **kwargs):
            ...
            context["total_site_deposit"] = self.get_total_site_investment()
            context["total_paid_profits"] = self.get_total_paid_profit()
            return context
        ...
        context.update(self.get_notifications_context()) #TODO: remove once done with the redis setup
        context["monthly_site_investments"] = self.get_monthly_site_investments()
        return context

    def get_profile_context(self)->dict[str, Any]:
        """
            Gathers profile-related data such as the user profile, referral link, account, and premium status.
    ...
    def get_account_context(self, **kwargs):
        """"
        Gathers account-related data such as balance, locked amount, available amount, total site deposit, and total paid profits.
    ...
    def get_transactions_context(self):
        """"
        Gathers transaction-related data such as transactions, monthly profit, and daily profit.
