import os
import time
from langchain.chains import (
    MapReduceDocumentsChain, 
    LLMChain, 
    ReduceDocumentsChain, 
    StuffDocumentsChain
)
import requests, json
from langchain_community.llms.ctransformers import CTransformers
from langchain.prompts import PromptTemplate
from langchain.text_splitter import RecursiveCharacterTextSplitter
from langchain_community.document_loaders import TextLoader

from amiribd.articles.editor.transcribe import TRANSCRIPT_DOWNLOAD_DIR, generate_random_file_name

class TranscriptSummarizer(object):
    def __init__(self,filename):
        self.filename_url = filename
        self.config = {'max_new_tokens': 4096, 'temperature': 0.7, 'context_length': 4096}
        self.local_filepath = ''
    # Function to download the PDF file locally
    def download_pdf(self):
        instance_file_name = generate_random_file_name()
            # Ensure the directory exists
        if not os.path.exists(TRANSCRIPT_DOWNLOAD_DIR):
            os.makedirs(TRANSCRIPT_DOWNLOAD_DIR)
        # Path for the local file
        local_file_path = os.path.join(TRANSCRIPT_DOWNLOAD_DIR, f"{instance_file_name}.txt")
        try:
            response = requests.get(self.filename_url, stream=True)
            response.raise_for_status()  # Raise an exception for HTTP errors
            with open(local_file_path, 'wb') as f:
                for chunk in response.iter_content(chunk_size=8192):
                    f.write(chunk)
            print(f"PDF downloaded successfully to {local_file_path}.")
            print(f"File size: {os.path.getsize(local_file_path)} bytes")
            self.local_filepath = local_file_path
            return local_file_path
        except requests.exceptions.RequestException as e:
            print(f"Failed to download PDF: {e}")
            return False

    def text_loader_to_docs(self):
        if not self.download_pdf():
            raise ValueError("Could not process the file")
        else:
            print("start the template loader")
            loader = TextLoader(self.local_filepath)
            print("finished the template loader")
            return loader.load()
    
    def prepare_llm(self):
        return CTransformers(
            model="TheBloke/Mistral-7B-Instruct-v0.1-GGUF",
            model_file="mistral-7b-instruct-v0.1.Q4_K_M.gguf",
            config=self.config,
            threads=os.cpu_count()
        )
    def prepare_map_template_and_chain(self):
        # Map template and chain
        docs = self.text_loader_to_docs()
        map_template = """<s>[INST] The following is a part of a transcript:
        {docs}
        Based on this, please identify the main points.
        Answer:  [/INST] </s>"""
        map_prompt = PromptTemplate.from_template(map_template)
        map_chain = LLMChain(llm=self.prepare_llm(), prompt=map_prompt)
        print("prep map template")
        return map_chain
    
    def prepare_reduce_template_chain(self):
        # Reduce template and chain
        reduce_template = """<s>[INST] The following is set of summaries from the transcript:
        {doc_summaries}
        Take these and distill it into a final, consolidated summary of the main points.
        Construct it as a well organized summary of the main points and should be between 3 and 5 paragraphs.
        Answer:  [/INST] </s>"""
        reduce_prompt = PromptTemplate.from_template(reduce_template)
        reduce_chain = LLMChain(llm=self.prepare_llm(), prompt=reduce_prompt)
        print("prep reduce template")
        return reduce_chain
    
    def summarize(self):
        # Takes a list of documents, combines them into a single string, and passes this to an LLMChain
        print("start summarizing the documents")
        combine_documents_chain = StuffDocumentsChain(
            llm_chain=self.prepare_reduce_template_chain(), document_variable_name="doc_summaries"
        )
        print("complete combination")
        # Combines and iteratively reduces the mapped documents
        reduce_documents_chain = ReduceDocumentsChain(
            # This is final chain that is called.
            combine_documents_chain=combine_documents_chain,
            # If documents exceed context for `StuffDocumentsChain`
            collapse_documents_chain=combine_documents_chain,
            # The maximum number of tokens to group documents into.
            token_max=4000,
        )
        print("complete the redu chaine")
        # Combining documents by mapping a chain over them, then combining results
        map_reduce_chain = MapReduceDocumentsChain(
            # Map chain
            llm_chain=self.prepare_map_template_and_chain(),
            # Reduce chain
            reduce_documents_chain=reduce_documents_chain,
            # The variable name in the llm_chain to put the documents in
            document_variable_name="docs",
            # Return the results of the map steps in the output
            return_intermediate_steps=True,
        )
        print("map and reduce combined")

        # Split documents into chunks
        text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=4000, chunk_overlap=0
        )
        print("text splitted")
        docs = self.text_loader_to_docs()
        split_docs = text_splitter.split_documents(docs)
        # Run the chain
        start_time = time.time()
        result = map_reduce_chain.__call__(split_docs, return_only_outputs=True)
        print(f"Time taken: {time.time() - start_time} seconds")
        print(result)
        return result['output_text']







def download_pdf(filename):
    instance_file_name = generate_random_file_name()
        # Ensure the directory exists
    if not os.path.exists(TRANSCRIPT_DOWNLOAD_DIR):
        os.makedirs(TRANSCRIPT_DOWNLOAD_DIR)
    # Path for the local file
    local_file_path = os.path.join(TRANSCRIPT_DOWNLOAD_DIR, f"{instance_file_name}.txt")
    try:
        response = requests.get(filename, stream=True)
        response.raise_for_status()  # Raise an exception for HTTP errors
        with open(local_file_path, 'wb') as f:
            for chunk in response.iter_content(chunk_size=8192):
                f.write(chunk)
        print(f"PDF downloaded successfully to {local_file_path}.")
        print(f"File size: {os.path.getsize(local_file_path)} bytes")
        return local_file_path
    except requests.exceptions.RequestException as e:
        print(f"Failed to download PDF: {e}")




def summarize_transcript(filename):
    # Load transcript
    local_filename = download_pdf(filename)
    loader = TextLoader(local_filename)
    docs = loader.load()
    print(docs)
    # Load LLM
    config = {'max_new_tokens': 4096, 'temperature': 0.7, 'context_length': 4096}
    llm = CTransformers(
        model="TheBloke/Mistral-7B-Instruct-v0.1-GGUF",
        model_file="mistral-7b-instruct-v0.1.Q4_K_M.gguf",
        config=config,
        threads=os.cpu_count()
    )
    # Map template and chain
    map_template = """<s>[INST] The following is a part of a transcript:
    {docs}
    Based on this, please identify the main points.
    Answer:  [/INST] </s>"""
    map_prompt = PromptTemplate.from_template(map_template)

    print("map template: " + map_template)
    map_chain = LLMChain(llm=llm, prompt=map_prompt)
    # Reduce template and chain
    reduce_template = """<s>[INST] The following is set of summaries from the transcript:
    {doc_summaries}
    Take these and distill it into a final, consolidated summary of the main points.
    Construct it as a well organized summary of the main points and should be between 3 and 5 paragraphs.
    Answer:  [/INST] </s>"""
    reduce_prompt = PromptTemplate.from_template(reduce_template)
    reduce_chain = LLMChain(llm=llm, prompt=reduce_prompt)


    print("reduce_chain", reduce_chain)


    # Takes a list of documents, combines them into a single string, and passes this to an LLMChain
    combine_documents_chain = StuffDocumentsChain(
        llm_chain=reduce_chain, document_variable_name="doc_summaries"
    )
    # Combines and iteratively reduces the mapped documents
    reduce_documents_chain = ReduceDocumentsChain(
        # This is final chain that is called.
        combine_documents_chain=combine_documents_chain,
        # If documents exceed context for `StuffDocumentsChain`
        collapse_documents_chain=combine_documents_chain,
        # The maximum number of tokens to group documents into.
        token_max=4000,
    )
    # Combining documents by mapping a chain over them, then combining results
    map_reduce_chain = MapReduceDocumentsChain(
        # Map chain
        llm_chain=map_chain,
        # Reduce chain
        reduce_documents_chain=reduce_documents_chain,
        # The variable name in the llm_chain to put the documents in
        document_variable_name="docs",
        # Return the results of the map steps in the output
        return_intermediate_steps=True,
    )

    print("map reduced and the chained combined ", map_reduce_chain)


    # Split documents into chunks
    text_splitter = RecursiveCharacterTextSplitter(
        chunk_size=4000, chunk_overlap=0
    )
    print("text splitter: ", text_splitter)
    split_docs = text_splitter.split_documents(docs)
    # Run the chain
    print("split_docs: ", split_docs)
    start_time = time.time()
    result = map_reduce_chain.__call__(split_docs, return_only_outputs=True)
    print(f"Time taken: {time.time() - start_time} seconds")
    print("summary result: ", result)
    print("============================================================")
    print("summary ", result['output_text'])
    print("============================================================")
    return result['output_text']


